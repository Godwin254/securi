...
export function SharedNavbar({firstname, lastname, estateName, userId, role}) {

  const link = role === 'admin' ? `/admin/${userId}/edit-profile` : `/user/${userId}/edit-profile`
  ...
  return (
    <div className='flex flex-row shadow-sm bg-white items-center px-16 py-5 mb-3'>
      ...
      <Link to={link} className='flex flex-row items-center ml-auto text-sm font-regular'>
